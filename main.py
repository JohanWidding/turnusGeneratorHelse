...
langvakter = ['Alice', 'Bernt', "Paul", 'Bob', 'David', 'Helena']
ordinærvakter = ['Eva', 'Finn', 'Greta', 'Lars', 'Stian', 'Fredrik', 'Jens']
nattvakter = ['Hannah', 'Ian', 'Joben', 'Jack', 'Katie', 'Liam', 'Ellen']
...
combinations = combinations_with_replacement((random.sample(shifts_keys, N)), num_days)
...
for combination in tqdm(combinations, desc="Processing", unit=" combinations"):
    ...
    name_counts_langvakt = Counter(name for name in all_names if name in langvakter)
    print(name_counts_langvakt)
...
for i, combination in enumerate(valid_combinations, 1):
    ...
    for shift in combination:
        ...
        workers_working[vakter[3][1:]].append((t+30, 15))
